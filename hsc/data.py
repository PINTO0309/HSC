...
class Sample:
    """Lightweight container describing a single cropped smile frame."""
...
POSITIVE_LABELS = {"smiling", "smile"}
NEGATIVE_LABELS = {"no_smiling", "not_smiling", "neutral"}
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", "smiling" if label == 1 else "no_smiling"))
...
def split_samples(
    ...
    for split, subset in assignments.items():
        ...
        logger.info(
            "Split '%s': %d samples from %d sources (%.1f%% labelled 'smiling').",
            split,
            total,
...
class HSCDataset(Dataset):
    """Torch dataset that reads smile crops from disk or embedded bytes."""
