...
from .model import HSC, ModelConfig

LOGGER = logging.getLogger("hsc")

LABEL_MAP = {0: "no_smiling", 1: "smiling"}
                ...
                results.append(
                    {
                        ...
                        "prob_smiling": float(probs[idx].detach().cpu().item()),
                    }
                )
    return results
...
def train_pipeline(config: TrainConfig, verbose: bool = False) -> Dict[str, Any]:
    ...
    train_dataset = HSCDataset(train_samples, transform=train_transform)
    val_dataset = HSCDataset(splits["val"], transform=eval_transform) if splits["val"] else None
    test_dataset = HSCDataset(splits["test"], transform=eval_transform) if splits["test"] else None
    ...
    model = HSC(model_config).to(device)
    base_metadata = {
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        epoch_path = config.output_dir / f"hsc_epoch_{epoch:04d}.pt"
        torch.save(epoch_payload, epoch_path)
        _prune_checkpoints(config.output_dir, "hsc_epoch_", 10)
        ...
        if score_value > best_f1:
            ...
            best_checkpoint_path = config.output_dir / f"hsc_best_epoch{epoch:04d}_f1_{score_value:.4f}.pt"
            best_state["checkpoint_path"] = str(best_checkpoint_path)
            torch.save(best_checkpoint, best_checkpoint_path)
            _prune_checkpoints(config.output_dir, "hsc_best_", 10)
    ...
    try:
        with torch.no_grad():
            while True:
                ...
                logits = model(tensor)
                prob_smiling = torch.sigmoid(logits)[0].item()

                label = LABEL_MAP[int(prob_smiling >= 0.5)]
                color = (0, 200, 0) if label == "smiling" else (50, 50, 255)
                cv2.putText(
                    frame,
                    f"Prob smiling: {prob_smiling:.2%}",
                    (12, 28),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.8,
                    color,
...
def export_to_onnx(
    ...
    torch.onnx.export(
        ...
        output_path,
        input_names=["images"],
        output_names=["prob_smiling"],
        dynamic_axes={"images": {0: "batch"}, "prob_smiling": {0: "batch"}},
        opset_version=opset,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="HSC training and inference pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the HSC binary classifier.")
    ...
    webcam_parser.add_argument("--window_name", type=str, default="HSC Webcam")
    ...
    webcam_onnx_parser.add_argument("--model", type=Path, required=True, help="Path to the exported HSC ONNX model.")
    webcam_onnx_parser.add_argument("--camera_index", type=int, default=0, help="OpenCV camera index (default: 0).")
    webcam_onnx_parser.add_argument("--window_name", type=str, default="HSC Webcam (ONNX)")
